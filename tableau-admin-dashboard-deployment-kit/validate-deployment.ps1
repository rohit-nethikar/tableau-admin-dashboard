...
if ($python) {
    ...
    if (-not $SkipImportTest -and $failures.Count -eq 0) {
        ...
        try {
            if ($python -eq "python") {
                $output = & python -c "import app; print('HEALTH_ROUTE=' + str(any(r.rule == '/health' for r in app.app.url_map.iter_rules())))" 2>&1
            } else {
                $output = & py -c "import app; print('HEALTH_ROUTE=' + str(any(r.rule == '/health' for r in app.app.url_map.iter_rules())))" 2>&1
            }

            $healthLine = $output | Where-Object { $_ -match '^HEALTH_ROUTE=' } | Select-Object -Last 1
            if ($healthLine -eq "HEALTH_ROUTE=True") {
                Write-Host "[OK] /health is registered" -ForegroundColor Green
            } else {
                Write-Host "[FAIL] /health was not found in Flask URL map" -ForegroundColor Red
                $failures.Add("/health was not found in Flask URL map")
            }
        } catch {
